...
export function respond(event, cb) {
  debug('= getAggregatedClicks.action', JSON.stringify(event));
  decrypt(event.authToken).then(async () => {
    if (event.start && event.campaignId) {
      try {
        const report = await GetTimeSeriesDataService.run(ClickReport, event.campaignId, event.start, event.end);
        return cb(null, report);
      } catch (err) {
        debug('= getAggregatedClicks.action', 'Error getting clicks report', err);
